        ...
        <div className="flex h-full flex-col bg-[var(--bg-panel)]">
            ...
            <ScrollArea className="flex-1">
                <div className="flex flex-col">
                    <button type="button" onClick={() => setShowPhoneInput(!showPhoneInput)}
                        className="flex items-center gap-3 px-3 py-2.5 text-left hover:bg-[var(--bg-hover)]">
                    ...
                    {showPhoneInput && (
                        <div className="border-b border-[var(--border-soft)] bg-[var(--bg-panel-2)] px-3 py-3">
                            <label htmlFor="phone-input" className="mb-1 block text-[10px] uppercase tracking-wide text-[var(--fg-tertiary)]">Numero di telefono (E.164)</label>
                            ...
                            <div className="mt-1.5 flex items-center justify-between">
                                {phone.length === 0 ? (
                                    <span className="text-[10px] text-[var(--fg-tertiary)]">Formato internazionale +XX...</span>
                                ) : isValidE164 ? (
                                    <span className="flex items-center gap-1 text-[10px] text-[var(--accent)]"><Check className="h-3 w-3" /> Valido</span>
                                ) : (
                                    <span className="flex items-center gap-1 text-[10px] text-[var(--status-failed)]"><AlertCircle className="h-3 w-3" /> Formato non valido</span>
                                )}
                                <button type="button" disabled={!isValidE164}
                                    className={cn("flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-medium", isValidE164 ? "bg-[var(--accent)] text-[var(--accent-fg)]" : "cursor-not-allowed bg-[var(--bg-panel)] text-[var(--fg-tertiary)]")}>
                                    Avvia
                                </button>
                            </div>
                            {outsideWindow && (
                                <div className="mt-2 flex items-center gap-1.5 rounded-md bg-[var(--bg-bubble-system)] px-2 py-1 text-[10px] text-[var(--fg-secondary)]">
                    ...
                    {mockContacts.map((c) => (
                        <button key={c.id} type="button" className="flex items-center gap-3 px-3 py-2.5 text-left hover:bg-[var(--bg-hover)]">

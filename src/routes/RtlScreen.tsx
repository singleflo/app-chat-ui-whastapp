...
export function RtlScreen() {
    return (
        <div className="flex h-full items-center justify-center bg-(--bg-app) p-4">
            <div
                className="flex h-[700px] w-[400px] flex-col overflow-hidden rounded-lg border border-(--border-strong) bg-(--bg-panel) shadow-(--shadow-overlay)"
                dir="rtl"
            >
                <header className="flex h-14 shrink-0 items-center gap-2 border-b border-(--border-strong) bg-(--bg-header) px-3">
                    <Avatar className="h-9 w-9">
                        <AvatarFallback style={{ backgroundColor: colorFromString("أحمد") }}>
                            أح
                        </AvatarFallback>
                    </Avatar>
                ...
                <div className="chat-doodle-bg relative flex-1 overflow-hidden">
                    <ScrollArea className="relative h-full">
                        <div className="mx-auto flex max-w-md flex-col gap-1 p-4" dir="rtl">
                            <div className="mx-auto my-2 rounded-md bg-(--bg-bubble-system) px-3 py-0.5 text-[11px] font-semibold text-(--fg-secondary)">
                                اليوم
                            </div>
                            <RtlBubble side="in">
                                <RtlText text="مرحبا! أريد الاستفسار عن الطلب رقم *#S00042*" />
                            </RtlBubble>
                            ...
                            <RtlBubble side="in">
                                <RtlText text="ممتاز! شكراً جزيلاً 👍" />
                            </RtlBubble>
                            <div className="mx-auto my-1 rounded-md bg-(--bg-bubble-system) px-2.5 py-1 text-[11px] text-(--fg-secondary)">
                ...
                <div className="shrink-0 bg-(--bg-panel-2) px-3 py-2" dir="rtl">
                    <div className="flex items-end gap-2 rounded-lg bg-(--bg-panel) px-2 py-1.5">
                        <button
                            type="button"
                            aria-label="رموز تعبيرية"
                            className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full text-(--fg-secondary) transition-all duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] hover:bg-(--bg-hover) hover:text-(--fg-primary) focus-visible:ring-2 focus-visible:ring-(--ring) focus-visible:ring-offset-2 focus-visible:ring-offset-(--bg-panel) focus-visible:outline-none active:scale-[0.97]"
                        >
            ...
            <div className="absolute bottom-4 left-4 rounded-lg bg-(--bg-panel) px-3 py-2 text-[11px] text-(--fg-tertiary) shadow-(--shadow-panel)">
                <div className="font-semibold text-(--fg-secondary)">RTL — Arabic demo</div>
...
function RtlBubble({
    ...
    return (
        <div
            className={cn(
                "relative max-w-[78%] rounded-[10px] px-2 py-1.5 text-sm shadow-(--shadow-bubble)",
                side === "in"
                    ? "self-start rounded-tr-[2px] bg-(--bg-bubble-in)"
                    : "self-end rounded-tl-[2px] bg-(--bg-bubble-out)"
            )}
        >
            {author && (
                <div className="mb-0.5 text-[11px] font-semibold text-(--accent)">{author}</div>
            )}
            {children}
        </div>
    );
...
function RtlText({ text }: { text: string }) {
    ...
    return (
        <span className="leading-snug whitespace-pre-wrap text-(--fg-primary)" dir="auto">
            {parts.map((p, i) => {
...
function RtlMeta({ ts, ack }: { ts: string; ack: "read" | "delivered" }) {
    return (
        <span className="float-left mt-1 ml-2 flex items-center gap-0.5 text-[11px] text-(--fg-tertiary) tabular-nums">
            {ts}
            {ack === "read" && (
                <CheckCheck className="h-3 w-3" style={{ color: "var(--color-ack-blue)" }} />
            )}
            {ack === "delivered" && <CheckCheck className="h-3 w-3 text-(--fg-tertiary)" />}
        </span>
    );
}

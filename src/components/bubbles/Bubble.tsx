    ...
    return (
        ...
        >
            ...
            {sender.kind === "api" && sender.source && (
                <span className="rounded bg-black/10 px-1 text-[9px] opacity-70">
                    📱 {sender.source}
                </span>
            )}
        </div>
    ...
    return (
        <div
            className={cn(
                ...
                tone === "note" &&
                    "self-end border border-(--fg-warning)/40 bg-(--bg-bubble-internal-note)",
                tone === "error" && "self-end bg-(--bg-bubble-error)",
                tone === "fallback" && "self-end bg-(--bg-bubble-fallback)",
                tone === "deleted" && "self-start bg-(--bg-bubble-deleted)",
                tone === "bot" &&
                    "bot-bubble-pattern self-end border border-(--fg-link)/20 bg-(--bg-bubble-bot)",
...
export function Reactions({ reactions }: { reactions?: Message["reactions"] }) {
    ...
    return (
        <div className="absolute right-2 -bottom-2.5 z-10 flex gap-0.5">
            {reactions.map((r) => (
                <span
                    key={r.emoji}
                    className={cn(

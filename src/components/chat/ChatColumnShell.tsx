    ...
    return (
        <header className="flex h-14 shrink-0 items-center gap-2 overflow-hidden border-b border-(--border-strong) bg-(--bg-header) px-3">
            ...
            <div className="flex shrink-0 items-center gap-0.5">
                <HeaderIcon label="Videochiamata">
                    <Video className="h-4 w-4" />
                </HeaderIcon>
                <HeaderIcon label="Chiamata">
                    <Phone className="h-4 w-4" />
                </HeaderIcon>
                <HeaderIcon label="Cerca nella chat">
                    <Search className="h-4 w-4" />
                </HeaderIcon>
                <HeaderIcon label="Altre opzioni">
...
function HeaderIcon({ children, label }: { children: React.ReactNode; label?: string }) {
    return (
        <button
            type="button"
            aria-label={label}
            className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-full text-(--fg-secondary) transition-all duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] hover:bg-(--bg-hover) focus-visible:ring-2 focus-visible:ring-(--ring) focus-visible:ring-offset-2 focus-visible:ring-offset-(--bg-panel) focus-visible:outline-none active:scale-[0.97]"
        >
            {children}
        </button>
    );
    ...
    if (conv.unassigned) {
        return (
            <Badge variant="outline" className="gap-1 border-(--unassigned) text-(--unassigned)">
                <span className="h-1.5 w-1.5 rounded-full bg-(--unassigned)" /> Non assegnata
            </Badge>
        );
    }

...
> Fonte di verità dei tipi: webhook field `messages` della Cloud API. Tipi ricevibili documentati da Meta: *text, audio, button, contacts, document, edit, errors, group, image, interactive, location, order, reaction, revoke, status, sticker, system, unsupported, video*. La UI DEVE avere un rendering per ognuno (anche solo fallback).
...
2. Testo inbound con *formattazione* completa + link preview card
